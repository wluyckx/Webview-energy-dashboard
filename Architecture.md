...
| R12 | **The design reference is vendor-hosted, not captured.** ADR-010 records the design as "captured in ~2,300 reviewed lines", but local `lovable/` holds only three staged files (`types/energy.ts`, `lib/energy-format.ts`, `hooks/useEnergyData.tsx`). `PowerFlow.tsx`, `primitives.tsx`, `KpiStrip.tsx`, `mock-energy.ts`, `Skeletons.tsx`, `StatusBar.tsx`, `EnergyBalanceCard.tsx`, `CostStub.tsx`, `AnimatedNumber.tsx`, `styles.css` and the four Recharts views exist **only in the Lovable cloud project** — an account, a credit balance, or a vendor product decision away from gone, while ten port stories depend on reading them | **Open, act early.** Mitigation: RW-C02 stages every portable file plus `styles.css` locally with a manifest, staging nothing ADR-010 discards and no credential. Cheap; do it alongside the R1 capture |
