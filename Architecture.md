...
| R8 | Deploy provenance mismatch (ghcr image pull vs on-VPS build) | **Mitigated for manual deploys 2026-07-30**: the compose `build:` section is restored, so `deploy.sh` provably ships what it builds — verified end-to-end by hash chain (local build = image contents = served bytes) on the 2026-07-30 deployment (`docs/stories/DEPLOY-2026-07-30.md`). The absent registry-publishing pipeline stays absent (archive-bound repo); full closure still comes with decommission (E6) |
